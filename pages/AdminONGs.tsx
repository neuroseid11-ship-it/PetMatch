...
const AdminONGs: React.FC = () => {
  ...
  const loadData = async () => {
    const data = await ongService.getAll();
    setOngs(data);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await ongService.save(formData, editingId || undefined);
      await loadData();
      setIsModalOpen(false);
    } catch (error) {
      console.error("Error saving ONG:", error);
      alert("Erro ao salvar ONG. Tente novamente.");
    }
  };
